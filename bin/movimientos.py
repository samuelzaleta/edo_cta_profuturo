...
with define_extraction(phase, area, postgres_pool, postgres_pool) as (postgres, _):
    ...
    with register_time(postgres_pool, phase, term_id, user, area):
        ...
        FROM TTAFOGRAL_MOV_AVOL DT
            LEFT JOIN (
                SELECT FNN_ID_REFERENCIA, FTC_FOLIO, FND_FECHA_VALOR_RCV, FND_FECHA_PAGO,
                       FNN_ULTIMO_SALARIO_INT_PER, FNN_DIAS_COTZDOS_BIMESTRE, FNN_DIAS_AUSENT_BIMESTRE,
                       FNN_DIAS_INCAP_BIMESTRE, FNC_RFC_PATRON, FNC_REG_PATRONAL_IMSS, FNN_SECLOT, FND_FECTRA,
                       ROW_NUMBER() OVER (PARTITION BY FNN_ID_REFERENCIA, FTC_FOLIO ORDER BY FND_FEH_CRE) AS ROW_NUM
                FROM CIERREN.TNAFORECA_SUA
            ) SUA ON SUA.FTC_FOLIO = DT.FTC_FOLIO AND SUA.FNN_ID_REFERENCIA = DT.FNN_ID_REFERENCIA
