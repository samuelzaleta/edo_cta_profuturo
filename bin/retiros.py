...
with define_extraction(phase, area, postgres_pool, postgres_pool) as (postgres, _):
    ...
    with register_time(postgres_pool, phase, term_id, user, area):
        ...
        WITH LIQ_SOLICITUDES AS (
            ...
            FROM (
                ...
                SELECT L.FTN_NUM_CTA_INVDUAL AS FCN_CUENTA, L.FTC_FOLIO AS FTC_FOLIO,
                       ...
                       L.TMC_DESC_NCI AS FTC_TMC_DESC_NCI, L.TMN_CVE_NCI AS FTN_TMN_CVE_NCI,
                       NULL AS "FTN_FEH_INI_PEN", NULL AS "FTN_FEH_RES_PEN", L.FTD_FEH_CRE
                FROM LIQ_SOLICITUDES L
        ...
        SELECT FTC_FOLIO, FTC_FOLIO_REL, FTN_NUM_CTA_INVDUAL, 
               ...
               SUM(ROUND(FTF_MONTO_PESOS, 2)) AS FTF_MONTO_PESOS
        FROM (
        ...
        GROUP BY FTC_FOLIO, FTC_FOLIO_REL, FTN_NUM_CTA_INVDUAL,
                 CASE WHEN FCN_ID_TIPO_SUBCTA IN (15, 16, 17, 18) THEN 1 ELSE 0 END
        """
        ...
        WITH RETIROS AS (
        ...
        ), SALDOS_AL_LIQUIDACION AS (
            SELECT FCN_CUENTA, FTN_TIPO_AHORRO, FTC_TMC_DESC_ITGY, SUM(FTF_SALDO_DIA) AS FTF_SALDO_DIA
            FROM (
                ...
                FROM cierren.thafogral_saldo_historico_v2 SH
                    ...
                    INNER JOIN (
                        SELECT DISTINCT SH.FTN_NUM_CTA_INVDUAL, SH.FCN_ID_TIPO_SUBCTA, 
                               SH.FCN_ID_SIEFORE, MAX(TRUNC(SH.FTD_FEH_LIQUIDACION)) AS FTD_FEH_LIQUIDACION
                        FROM CIERREN.THAFOGRAL_SALDO_HISTORICO_V2 SH
                            INNER JOIN RETIROS RET ON SH.FTN_NUM_CTA_INVDUAL = RET.FCN_CUENTA
            ...
            GROUP BY FCN_CUENTA, FTN_TIPO_AHORRO, FTC_TMC_DESC_ITGY
        ), SALDOS_INICIO_MES AS (
            SELECT SH.FTN_NUM_CTA_INVDUAL AS FCN_CUENTA, 1 AS FTN_TIPO_AHORRO, RET.FTC_TMC_DESC_ITGY,
            ...
            SELECT FTN_NUM_CTA_INVDUAL AS FCN_CUENTA, FTC_TMC_DESC_ITGY, FTN_TIPO_AHORRO,
                   SUM(PESOS) AS FTF_SALDO_DIA
            FROM (
                SELECT FTN_NUM_CTA_INVDUAL, FCN_ID_TIPO_SUBCTA, r.FTC_TMC_DESC_ITGY,
                       CASE WHEN FCN_ID_TIPO_SUBCTA IN (15, 16, 17, 18) THEN 1 ELSE 0 END FTN_TIPO_AHORRO,
                       FCN_ID_SIEFORE, SUM(FTN_DIA_PESOS) PESOS
            ...
            GROUP BY FTN_NUM_CTA_INVDUAL, FTC_TMC_DESC_ITGY, FTN_TIPO_AHORRO
        )
        SELECT FCN_CUENTA, FTN_TIPO_AHORRO, FTC_TMC_DESC_ITGY, SUM(FTF_SALDO_DIA) AS FTF_SALDO_DIA
        FROM (
            SELECT FCN_CUENTA, FTN_TIPO_AHORRO, FTC_TMC_DESC_ITGY, FTF_SALDO_DIA
        ...
        df = spark.sql("""
        SELECT DISTINCT RET.FCN_CUENTA, RET.FTC_FOLIO, SL.FTN_TIPO_AHORRO, RET.FCN_ID_PROCESO,
               RET.FCN_ID_SUBPROCESO, RET.FTC_TPSEGURO, RET.FTC_REGIMEN, RET.FTC_TPPENSION,
        ...
        SELECT DISTINCT RET.FCN_CUENTA, RET.FTC_FOLIO, SL.FTN_TIPO_AHORRO, RET.FCN_ID_PROCESO,
               ...
               RET.FTN_ARCHIVO, RET.FTC_LEY_PENSION, RET.FTC_FON_ENTIDAD, RET.FTD_FEH_CRE
        FROM RETIROS RET
            INNER JOIN SALDOS_LIQUIDACIONES SL ON RET.FTC_FOLIO = SL.FTC_FOLIO
        ...
        df = spark.sql("""
        SELECT SL.FCN_CUENTA, SL.FTC_FOLIO, SL.FTN_TIPO_AHORRO, SL.FCN_ID_PROCESO,
               SL.FCN_ID_SUBPROCESO, SL.FTC_TPSEGURO, SL.FTC_REGIMEN, SL.FTC_TPPENSION,
               SL.FTF_MONTO_LIQ, SL.FTN_ISR_LIQ, SI.FTF_SALDO_DIA AS FTF_SALDO_INICIAL,
               SL.FCC_TIPO_BANCO, SL.FCC_MEDIO_PAGO, SL.FTC_TMC_DESC_ITGY, SL.FTC_TMC_DESC_NCI,
               SL.FTN_TMN_CVE_NCI, SL.FTN_FEH_INI_PEN, SL.FTN_FEH_RES_PEN, SL.FTN_ARCHIVO,
               SL.FTC_LEY_PENSION, SL.FTC_FON_ENTIDAD, SL.FTD_FEH_CRE
        FROM SALDOS_LIQUIDACIONES SL
            FULL JOIN SALDOS_INICIALES SI ON SI.FCN_CUENTA = SL.FCN_CUENTA
