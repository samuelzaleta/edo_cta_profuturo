...
with define_extraction(phase, area, postgres_pool, postgres_oci_pool) as (postgres, postgres_oci):
    ...
    with register_time(postgres_pool, phase, term_id, user, area):
        ...
        SELECT DISTINCT X.FTN_NUM_CTA_INVDUAL, X.FTC_FOLIO, X.FTC_FOLIO_REL, PT.TMC_DESC_ITGY,
        ...
        AND (TRUNC(resol.FTD_FEH_CRE),resol.FTC_NSS) IN (SELECT MAX(TRUNC(resol.FTD_FEH_CRE)),resol.FTC_NSS FROM TTAFORETI_RESOLUCIONES resol GROUP BY resol.FTC_NSS)
        )
        , DIS_TRANS AS (
        ...
        SELECT DISTINCT L.FTN_NUM_CTA_INVDUAL AS FCN_CUENTA, L.FTC_FOLIO AS FTC_FOLIO,
               ...
               coalesce(resol.FTC_CVE_TIPO_SEG,T.FTC_CVE_TIPO_SEG) FTC_TPSEGURO, coalesce(resol.FTC_CVE_REGIMEN,T.FTC_CVE_REGIMEN) FTC_REGIMEN,
        ...
        SELECT DISTINCT L.FTN_NUM_CTA_INVDUAL AS FCN_CUENTA, L.FTC_FOLIO AS FTC_FOLIO,
               ...
               coalesce(resol.FTC_CVE_TIPO_SEG,TR.FCC_TPSEGURO) FTC_TPSEGURO, coalesce(resol.FTC_CVE_REGIMEN,TR.FTC_REGIMEN) FTC_REGIMEN,
        ...
        FROM (
            ...
            WHERE --ttcp.FTD_FEH_CRE BETWEEN :start AND :end
              --AND
               (FTC_FOLIO, FTN_ID_ASOCIADO, FTN_NUM_REEXP) IN (
        ...
        SELECT DISTINCT X.FCN_CUENTA, X.FTC_FOLIO, X.FTC_FOLIO_REL, FCN_ID_PROCESO, FCN_ID_SUBPROCESO,
        ...
        GROUP BY FCN_CUENTA,FTN_TIPO_AHORRO,
            ...
            FROM (
                ...
                WHERE trunc(q.FTD_FEH_CRE) < trunc(r.FTD_FEH_CRE)
        ...
        print('RETIROS')
        spark.sql("SELECT COUNT(*) FROM RETIROS").show(40)
        ...
        spark.sql("SELECT COUNT(*) FROM SALDOS_LIQUIDACIONES").show(40)
        ...
        spark.sql("SELECT COUNT(*) FROM SALDOS_LIQUIDACIONES_TRANSFERENCIAS").show(40)
        ...
        SELECT DISTINCT RET.FCN_CUENTA, RET.FTC_FOLIO, RET.FCN_ID_PROCESO,
               ...
               SL.SALDO_TRANSFERIDO_AHORRORET AS FTN_SALDO_TRANSFERIDO_AHORRORET,
        ...
        WHERE RET.FTC_TMC_DESC_ITGY IN ( 'T73' ,'TPP', 'T97', 'TPR', 'TED', 'RJP', 'TRE', 'TEX', 'TIX', 'TEI', 'TPI', 'TNI', 'TJI', 'PPI', 'RCI', 'TAI', 'TJU' )
