...
from profuturo.database import get_postgres_pool, get_mit_pool, get_mit_url, configure_postgres_spark, configure_mit_spark
...
with define_extraction(phase, postgres_pool, mit_pool) as (postgres, mit):
    ...
    with register_time(postgres_pool, phase, term=term_id):
        ...
        cuenta_list = spark.sql(" SELECT DISTINCT FCN_CUENTA FROM retiros").collect()

        read_table_insert_temp_view(
        configure_mit_spark, f"""
        ...
        INNER JOIN (
            ...
            WHERE SHMAX.FTD_FEH_LIQUIDACION <= {start_month}
        ...
        INNER JOIN (
            ...
            WHERE FCD_FEH_ACCION <= {start_month} AND SH.FTN_NUM_CTA_INVDUAL IN [{','.join(cuenta_list)}]
        ...
        select * 
        from retiros r 
        inner join saldos_retiros s 
        on r.FCN_CUENTA = s.FCN_CUENTA
        """)
        # Cifras de control
