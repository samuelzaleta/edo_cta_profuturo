    ...
    with register_time(postgres_pool, phase, term_id, user, area):
        ...
        def insertar_tablas():
            ...
            extract_dataset_spark(configure_postgres_spark, configure_postgres_oci_spark,
                                  """ SELECT * FROM "GESTOR"."TTGESPRO_CONFIGURACION_FORMATO_ESTADO_CUENTA" """,
                                  '"MAESTROS"."TTGESPRO_CONFIGURACION_FORMATO_ESTADO_CUENTA"'
                                  )

            extract_dataset_spark(configure_postgres_spark, configure_postgres_oci_spark,
                                  """ SELECT * FROM "GESTOR"."TCGESPRO_FORMATO_ESTADO_CUENTA" """,
                                  '"MAESTROS"."TCGESPRO_FORMATO_ESTADO_CUENTA"'
                                  )

            extract_dataset_spark(configure_postgres_spark, configure_postgres_oci_spark,
                                  """ SELECT * FROM "GESTOR"."TCGESPRO_PERIODICIDAD" """,
                                  '"MAESTROS"."TCGESPRO_PERIODICIDAD"'
                                  )
            ...
            extract_dataset_spark(configure_postgres_spark, configure_postgres_oci_spark,
                                  """ SELECT * FROM "GESTOR"."TCGESPRO_MUESTRA" """,
                                  '"MAESTROS"."TCGESPRO_MUESTRA"'
                                  )

            extract_dataset_spark(configure_postgres_spark, configure_postgres_oci_spark,
                                  """ SELECT * FROM "MAESTROS"."TCDATMAE_PENSION" """,
                                  '"MAESTROS"."TCDATMAE_PENSION"'
                                  )

            extract_dataset_spark(configure_postgres_spark, configure_postgres_oci_spark,
                                  """ SELECT * FROM "GESTOR"."TCGESPRO_INDICADOR_ESTADO_CUENTA"  """,
                                  '"MAESTROS"."TCGESPRO_INDICADOR_ESTADO_CUENTA"'
                                  )
            ...
            extract_dataset_spark(configure_postgres_spark, configure_postgres_oci_spark,
                                  """ SELECT * FROM "GESTOR"."TCGESPRO_CONFIGURACION_ANVERSO"  """,
                                  '"MAESTROS"."TCGESPRO_CONFIGURACION_ANVERSO"'
                                  )
            ...
            extract_dataset_spark(configure_postgres_spark, configure_postgres_oci_spark,
                                  """ SELECT * FROM "GESTOR"."TTGESPRO_MOV_PROFUTURO_CONSAR" """,
                                  '"MAESTROS"."TTGESPRO_MOV_PROFUTURO_CONSAR"'
                                  )

            extract_dataset_spark(configure_postgres_spark, configure_postgres_oci_spark,
                                  """ SELECT * FROM "GESTOR"."TCGESPRO_REFER_MOV_CONSAR" """,
                                  '"MAESTROS"."TCGESPRO_REFER_MOV_CONSAR"'
                                  )

            extract_dataset_spark(configure_postgres_spark, configure_postgres_oci_spark,
                                  """ SELECT * FROM "MAESTROS"."TCDATMAE_MOVIMIENTO_CONSAR" """,
        ...
        def eliminar_tablas():
            ...
            postgres_oci.execute(
                text(""" DROP TABLE IF EXISTS "MAESTROS"."TCGESPRO_FORMATO_ESTADO_CUENTA" """))

            postgres_oci.execute(
                text(""" DROP TABLE IF EXISTS "MAESTROS"."TTGESPRO_CONFIGURACION_FORMATO_ESTADO_CUENTA" """))

            postgres_oci.execute(
                text(""" DROP TABLE IF EXISTS "MAESTROS"."TCGESPRO_PERIODICIDAD" """))

            postgres_oci.execute(
                text(""" DROP TABLE IF EXISTS "MAESTROS"."TCGESPRO_PERIODO" """))

            postgres_oci.execute(
                text(""" DROP TABLE IF EXISTS "MAESTROS"."TCGESPRO_MUESTRA" """))

            postgres_oci.execute(
                text(""" DROP TABLE IF EXISTS "MAESTROS"."TCDATMAE_PENSION" """))

            postgres_oci.execute(
                text(""" DROP TABLE IF EXISTS "MAESTROS"."TCGESPRO_INDICADOR_ESTADO_CUENTA" """))

            postgres_oci.execute(
                text(""" DROP TABLE IF EXISTS "MAESTROS"."TCGESPRO_CONFIGURACION_ANVERSO" """))

            postgres_oci.execute(
                text(""" DROP TABLE IF EXISTS "MAESTROS"."TCDATMAE_TIPO_SUBCUENTA" """))

            postgres_oci.execute(
                text(""" DROP TABLE IF EXISTS "MAESTROS"."TTGESPRO_MOV_PROFUTURO_CONSAR" """))

            postgres_oci.execute(
                text(""" DROP TABLE IF EXISTS "MAESTROS"."TCGESPRO_REFER_MOV_CONSAR" """))

            postgres_oci.execute(
                text(""" DROP TABLE IF EXISTS "MAESTROS"."TCDATMAE_MOVIMIENTO_CONSAR" """))
        ...
        if not is_reprocess:
            ...
            SELECT DISTINCT CA."FCN_CUENTA", :term, CA."FTC_USUARIO_CARGA"::INT, :area, current_timestamp
            ...
            """), {"term": term_id, "area": area, "user": str(user)})
        ...
        WITH DATASET AS (
            ...
            M."FCN_ID_PERIODO",
            M."FCN_CUENTA",
            F."FCN_ID_INDICADOR_CLIENTE",
            F."FCN_ID_INDICADOR_AFILIACION",
            ...
            FROM
                "MAESTROS"."TTGESPRO_CONFIGURACION_FORMATO_ESTADO_CUENTA" F
            INNER JOIN
                    "MAESTROS"."TCGESPRO_FORMATO_ESTADO_CUENTA" FE ON F."FCN_ID_FORMATO_ESTADO_CUENTA" = FE."FTN_ID_FORMATO_ESTADO_CUENTA"
            INNER JOIN
                    "MAESTROS"."TCGESPRO_PERIODICIDAD" PG ON F."FCN_ID_PERIODICIDAD_GENERACION" = PG."FTN_ID_PERIODICIDAD"
            INNER JOIN
                    "MAESTROS"."TCGESPRO_PERIODICIDAD" PA ON F."FCN_ID_PERIODICIDAD_ANVERSO" = PA."FTN_ID_PERIODICIDAD"
            INNER JOIN
                    "MAESTROS"."TCGESPRO_PERIODICIDAD" PR ON F."FCN_ID_PERIODICIDAD_REVERSO" = PR."FTN_ID_PERIODICIDAD"
            INNER JOIN
                    "MAESTROS"."TCGESPRO_PERIODO" P ON P."FTN_ID_PERIODO" = :term
            INNER JOIN
                    "MAESTROS"."TCGESPRO_MUESTRA" M ON P."FTN_ID_PERIODO" = M."FCN_ID_PERIODO"
            LEFT JOIN
                    "MAESTROS"."TCDATMAE_PENSION" TP ON TP."FCN_CUENTA" = M."FCN_CUENTA"
            {filter_reprocessed_samples}
            WHERE
                mod(extract(MONTH FROM to_date(P."FTC_PERIODO", 'MM/YYYY')), PG."FTN_MESES") = 0
                AND F."FTB_ESTATUS" = true
                AND M."FCN_ID_USUARIO" = :user
            )
            SELECT DISTINCT
                D."FTN_ID_GRUPO_SEGMENTACION",
                D."FTC_CANDADO_APERTURA",
                D."FTN_ID_FORMATO",
                D."FCN_ID_PERIODO",
                C."FTN_CUENTA" AS "FCN_NUMERO_CUENTA",
                D."FTD_FECHA_CORTE",
                D."FTD_FECHA_GRAL_INICIO",
                D."FTD_FECHA_GRAL_FIN",
                D."FTD_FECHA_MOV_INICIO",
                D."FTD_FECHA_MOV_FIN",
                D."FTN_ID_SIEFORE",
                I."FTC_PERFIL_INVERSION" AS "FTC_DESC_SIEFORE",
                D."FTC_TIPOGENERACION",
                D."FTC_DESC_TIPOGENERACION",
                ...
                CASE WHEN  C."FTC_COLONIA" LIKE '%NO ASIGNADO%' THEN '' ELSE C."FTC_COLONIA" END AS "FTC_COLONIA",
                CASE
                ...
                END AS "FTC_DELEGACION",
                C."FTC_CODIGO_POSTAL" AS "FTN_CP",
                C."FTC_ENTIDAD_FEDERATIVA",
                C."FTC_NSS",
                C."FTC_RFC",
                C."FTC_CURP",
                I."FTC_TIPO_PENSION" AS "FTC_TIPO_PENSION",
                D."FTN_PENSION_MENSUAL",
                D."FTC_FORMATO",
                IEC."FTC_DESCRIPCION" AS "FTC_TIPO_TRABAJADOR",
                :user AS "FTC_USUARIO_ALTA",
                C."FTC_ANIO_NACIMIENTO"
            FROM DATASET D
            INNER JOIN
                "HECHOS"."TCHECHOS_CLIENTE" I ON D."FCN_ID_PERIODO" = I."FCN_ID_PERIODO"
                    ...
                    END
            INNER JOIN
                "MAESTROS"."TCDATMAE_CLIENTE" C ON I."FCN_CUENTA" = C."FTN_CUENTA"
            INNER JOIN
                "MAESTROS"."TCGESPRO_INDICADOR_ESTADO_CUENTA" IE ON IE."FTN_ID_INDICADOR_ESTADO_CUENTA" = D."FCN_ID_INDICADOR_CLIENTE"
                    ...
                    END
            INNER JOIN
                "MAESTROS"."TCGESPRO_INDICADOR_ESTADO_CUENTA" IEC ON IEC."FTN_ID_INDICADOR_ESTADO_CUENTA" = D."FCN_ID_INDICADOR_AFILIACION"
                    ...
                    END
            INNER JOIN
                "MAESTROS"."TCGESPRO_INDICADOR_ESTADO_CUENTA" TIEC ON TIEC."FTN_ID_INDICADOR_ESTADO_CUENTA" = D."FCN_ID_INDICADOR_BONO"
            ...
            FROM
                DATASET D
            INNER JOIN
                "HECHOS"."TCHECHOS_CLIENTE" I ON D."FCN_ID_PERIODO" = I."FCN_ID_PERIODO" AND D."FCN_CUENTA" = I."FCN_CUENTA"
                    AND D."FCN_ID_GENERACION" = CASE I."FTC_GENERACION"
        ...
        reverso_df = _create_spark_dataframe(spark, configure_postgres_oci_spark, f"""
        WITH data as (
        SELECT
            F."FCN_ID_FORMATO_ESTADO_CUENTA" AS "FTN_ID_FORMATO",
            M."FCN_CUENTA" AS "FCN_NUMERO_CUENTA",
            M."FCN_ID_PERIODO",
            M."FCN_CUENTA",
            F."FCN_ID_GENERACION",
            F."FCN_ID_INDICADOR_CLIENTE",
            F."FCN_ID_INDICADOR_AFILIACION",
            F."FCN_ID_INDICADOR_BONO"
        FROM "MAESTROS"."TTGESPRO_CONFIGURACION_FORMATO_ESTADO_CUENTA" F
            INNER JOIN
                "MAESTROS"."TCGESPRO_FORMATO_ESTADO_CUENTA" FE ON F."FCN_ID_FORMATO_ESTADO_CUENTA" = FE."FTN_ID_FORMATO_ESTADO_CUENTA"
            INNER JOIN
                "MAESTROS"."TCGESPRO_PERIODICIDAD" PG ON F."FCN_ID_PERIODICIDAD_GENERACION" = PG."FTN_ID_PERIODICIDAD"
        ...
        SELECT
            D."FTN_ID_FORMATO",
            D."FCN_NUMERO_CUENTA",
            concat(C."FTC_NSS",'-',C."FTC_RFC") AS "FTC_NSS_RFC"
        FROM data D
            ...
            INNER JOIN
                "MAESTROS"."TCDATMAE_CLIENTE" C ON I."FCN_CUENTA" = C."FTN_CUENTA"
            INNER JOIN
                "MAESTROS"."TCGESPRO_INDICADOR_ESTADO_CUENTA" IE
            ...
            INNER JOIN
                "MAESTROS"."TCGESPRO_INDICADOR_ESTADO_CUENTA" IEC
                ON IEC."FTN_ID_INDICADOR_ESTADO_CUENTA" = D."FCN_ID_INDICADOR_AFILIACION"
                AND IEC."FTN_VALOR" = CASE
                                   WHEN I."FTB_PENSION" THEN 1
                                   WHEN I."FTC_TIPO_CLIENTE" = 'Afiliado' THEN 714
                                   WHEN I."FTC_TIPO_CLIENTE" = 'Asignado' THEN 713 END
            INNER JOIN
                "MAESTROS"."TCGESPRO_INDICADOR_ESTADO_CUENTA" TIEC
                ON TIEC."FTN_ID_INDICADOR_ESTADO_CUENTA" = D."FCN_ID_INDICADOR_BONO"
                AND TIEC."FTN_VALOR" = CASE I."FTB_BONO"
                                   WHEN false THEN  1
                                   WHEN true THEN 2 END
            UNION ALL
        SELECT DISTINCT D."FTN_ID_FORMATO", D."FCN_NUMERO_CUENTA", concat(C."FTC_NSS",'-',C."FTC_RFC") AS "FTC_NSS_RFC"
        FROM data D
            INNER JOIN "HECHOS"."TCHECHOS_CLIENTE" I
            ON D."FCN_ID_PERIODO" = I."FCN_ID_PERIODO"
            AND D."FCN_CUENTA" = I."FCN_CUENTA"
            AND D."FCN_ID_GENERACION" = CASE I."FTC_GENERACION"
                                        WHEN 'DECIMO TRANSITORIO' THEN 1 END
            INNER JOIN "MAESTROS"."TCDATMAE_CLIENTE" C ON I."FCN_CUENTA" = C."FTN_CUENTA"
        ), periodos AS (
            SELECT
            F."FCN_ID_FORMATO_ESTADO_CUENTA",
            min(T."FTN_ID_PERIODO") AS PERIODO_INICIAL,
            max(T."FTN_ID_PERIODO") AS PERIODO_FINAL
            FROM
                "MAESTROS"."TTGESPRO_CONFIGURACION_FORMATO_ESTADO_CUENTA" F
            INNER JOIN
                "MAESTROS"."TCGESPRO_CONFIGURACION_ANVERSO" A ON F."FCN_ID_GENERACION" = A."FCN_GENERACION"
            INNER JOIN
                "MAESTROS"."TCGESPRO_PERIODICIDAD" P ON F."FCN_ID_PERIODICIDAD_GENERACION" = P."FTN_ID_PERIODICIDAD" AND mod(extract(MONTH FROM :end), P."FTN_MESES") = 0
            INNER JOIN
                "MAESTROS"."TCGESPRO_PERIODICIDAD" PA ON F."FCN_ID_PERIODICIDAD_REVERSO" = PA."FTN_ID_PERIODICIDAD"
            INNER JOIN
                    "MAESTROS"."TCGESPRO_PERIODO" T ON to_date(T."FTC_PERIODO", 'MM/YYYY') BETWEEN :end - INTERVAL '1 month' * PA."FTN_MESES" AND :end
        ...
        movimientos as (
            SELECT DISTINCT
                ...
                R."FCN_ID_PERIODO",
                CASE
                    WHEN R."FCN_ID_TIPO_MOVIMIENTO" = '180' THEN (R."FTF_MONTO_PESOS" * -1)
                    WHEN R."FCN_ID_TIPO_MOVIMIENTO" = '181' THEN R."FTF_MONTO_PESOS"
                ...
                0 AS "FTN_MONPES"
            FROM
                "HECHOS"."TTHECHOS_MOVIMIENTO" R
            INNER JOIN
                dataset M ON M."FCN_NUMERO_CUENTA" = R."FCN_CUENTA"
            INNER JOIN
                periodos ON R."FCN_ID_PERIODO" BETWEEN periodos.PERIODO_INICIAL AND periodos.PERIODO_FINAL AND M."FTN_ID_FORMATO" = periodos."FCN_ID_FORMATO_ESTADO_CUENTA"
            INNER JOIN
                "MAESTROS"."TCDATMAE_TIPO_SUBCUENTA" SB ON SB."FTN_ID_TIPO_SUBCTA" = R."FCN_ID_TIPO_SUBCTA"
            UNION ALL
            SELECT
                R."CSIE1_NUMCUE",
                R."FCN_ID_PERIODO",
                ...
                NULL AS "FTC_SUA_RFC_PATRON", SB."FTC_TIPO_CLIENTE" AS "TIPO_SUBCUENA", MP."FTN_MONPES"
            FROM "HECHOS"."TTHECHOS_MOVIMIENTOS_INTEGRITY" R
            INNER JOIN "MAESTROS"."TCGESPRO_MOVIMIENTO_PROFUTURO" MP
            ...
            SELECT DISTINCT 
                ...
                CASE 
                    ...
                    WHEN TRMC."FTB_FIJA" = false 
                             AND TRMC."FTC_REFERENCIA_VARIABLE" LIKE '%R.F.C. <RFC PATRÓN>1%'
                        THEN concat(R."FCN_ID_PERIODO",'-',R."FTC_SUA_RFC_PATRON")
                    WHEN TRMC."FTB_FIJA" = false AND TRMC."FTC_REFERENCIA_VARIABLE" LIKE '%IMSS/ISSSTE%'
                        THEN concat(R."FCN_ID_PERIODO",'-',R."TIPO_SUBCUENA")
                    WHEN TRMC."FTB_FIJA" = false AND TRMC."FTC_REFERENCIA_VARIABLE" LIKE '%NSS-CURP%'
                        THEN concat(R."FCN_ID_PERIODO",'-',D."FTC_NSS_RFC")
                END AS "FTC_PERIODO_REFERENCIA",
                R."FTF_MONTO_PESOS" AS "FTN_MONTO",
                CASE
                    WHEN MC."FTB_INTEGRACION_DIAS_COTIZADOS_SALARIO_BASE" THEN R."FTN_SUA_DIAS_COTZDOS_BIMESTRE"
                END AS "FTN_DIA_COTIZADO",
                CASE
                    WHEN MC."FTB_INTEGRACION_DIAS_COTIZADOS_SALARIO_BASE"
                        THEN R."FTN_SUA_ULTIMO_SALARIO_INT_PER"
                END AS "FTN_SALARIO_BASE",
                now() AS "FTD_FECHAHORA_ALTA", :user AS "FTC_USUARIO_ALTA"
            FROM 
                "MAESTROS"."TTGESPRO_CONFIGURACION_FORMATO_ESTADO_CUENTA" F
            INNER JOIN 
                "MAESTROS"."TCGESPRO_PERIODICIDAD" PG ON F."FCN_ID_PERIODICIDAD_REVERSO" = PG."FTN_ID_PERIODICIDAD"
            INNER JOIN 
                dataset D ON D."FTN_ID_FORMATO" = F."FCN_ID_FORMATO_ESTADO_CUENTA"
            INNER JOIN movimientos R ON D."FCN_NUMERO_CUENTA" = R."FCN_CUENTA"
            INNER JOIN 
                "MAESTROS"."TCGESPRO_PERIODO" T ON R."FCN_ID_PERIODO" = T."FTN_ID_PERIODO"
            INNER JOIN 
                "MAESTROS"."TTGESPRO_MOV_PROFUTURO_CONSAR" PC ON R."FCN_ID_CONCEPTO_MOVIMIENTO" = PC."FCN_ID_MOVIMIENTO_PROFUTURO"
                AND PC."FCN_MONPES" = R."FTN_MONPES" AND PC."FCN_ID_MOVIMIENTO_CONSAR" NOT IN (2,6,5,29,11,3)
            INNER JOIN 
                    "MAESTROS"."TCDATMAE_MOVIMIENTO_CONSAR" MC ON PC."FCN_ID_MOVIMIENTO_CONSAR" = MC."FTN_ID_MOVIMIENTO_CONSAR"
            LEFT JOIN 
                    "MAESTROS"."TCGESPRO_REFER_MOV_CONSAR"  TRMC ON MC."FCN_ID_REFERENCIA" = TRMC."FTN_ID_REFERENCIA"
            UNION ALL
            SELECT
                F."FCN_ID_FORMATO_ESTADO_CUENTA" AS "FTN_ID_FORMATO", 
                MC."FTC_MOV_TIPO_AHORRO" AS "FTC_SECCION",
            ...
            INNER JOIN 
                dataset D ON D."FTN_ID_FORMATO" = F."FCN_ID_FORMATO_ESTADO_CUENTA"
            INNER JOIN 
                movimientos R ON D."FCN_NUMERO_CUENTA" = R."FCN_CUENTA"
            INNER JOIN 
                "MAESTROS"."TCGESPRO_PERIODO" T ON R."FCN_ID_PERIODO" = T."FTN_ID_PERIODO"
            INNER JOIN 
                "MAESTROS"."TTGESPRO_MOV_PROFUTURO_CONSAR" PC ON R."FCN_ID_CONCEPTO_MOVIMIENTO" = PC."FCN_ID_MOVIMIENTO_PROFUTURO"
        ...
        anverso_df = _create_spark_dataframe(spark, configure_postgres_oci_spark, f"""
        WITH data as (
        SELECT
        DISTINCT
        ...
        F."FCN_ID_INDICADOR_BONO"
        FROM "MAESTROS"."TTGESPRO_CONFIGURACION_FORMATO_ESTADO_CUENTA" F
            INNER JOIN "MAESTROS"."TCGESPRO_FORMATO_ESTADO_CUENTA" FE ON F."FCN_ID_FORMATO_ESTADO_CUENTA" = FE."FTN_ID_FORMATO_ESTADO_CUENTA"
            INNER JOIN "MAESTROS"."TCGESPRO_PERIODICIDAD" PG ON F."FCN_ID_PERIODICIDAD_GENERACION" = PG."FTN_ID_PERIODICIDAD"
            INNER JOIN "MAESTROS"."TCGESPRO_PERIODICIDAD" PA ON F."FCN_ID_PERIODICIDAD_ANVERSO" = PA."FTN_ID_PERIODICIDAD"
            INNER JOIN "MAESTROS"."TCGESPRO_PERIODICIDAD" PR ON F."FCN_ID_PERIODICIDAD_REVERSO" = PR."FTN_ID_PERIODICIDAD"
            INNER JOIN "MAESTROS"."TCGESPRO_PERIODO" P ON P."FTN_ID_PERIODO" = :term
            INNER JOIN "MAESTROS"."TCGESPRO_MUESTRA" M ON P."FTN_ID_PERIODO" = M."FCN_ID_PERIODO"
            {filter_reprocessed_samples}
            -- QUITAR COMENT  CONDICION VALIDA PERO NO ES PERIODO CUATRIMESTRAL
            WHERE mod(extract(MONTH FROM to_date(P."FTC_PERIODO", 'MM/YYYY')), PG."FTN_MESES") = 0
            AND F."FTB_ESTATUS" = true
        ...
        FROM data D
            ...
            AND D."FCN_ID_GENERACION" = CASE I."FTC_GENERACION"
               ...
               WHEN 'MIXTO' THEN 4 END
            INNER JOIN "MAESTROS"."TCDATMAE_CLIENTE" C ON I."FCN_CUENTA" = C."FTN_CUENTA"
            INNER JOIN "MAESTROS"."TCGESPRO_INDICADOR_ESTADO_CUENTA" IE
            ON IE."FTN_ID_INDICADOR_ESTADO_CUENTA" = D."FCN_ID_INDICADOR_CLIENTE"
            AND IE."FTN_VALOR" = CASE I."FTC_ORIGEN"
               WHEN 'ISSSTE' THEN 67
               WHEN 'IMSS' THEN 66
               WHEN 'MIXTO' THEN 69
               WHEN 'INDEPENDIENTE' THEN 68 END
            INNER JOIN "MAESTROS"."TCGESPRO_INDICADOR_ESTADO_CUENTA" IEC
            ON IEC."FTN_ID_INDICADOR_ESTADO_CUENTA" = D."FCN_ID_INDICADOR_AFILIACION"
            AND IEC."FTN_VALOR" = CASE
               WHEN I."FTB_PENSION" THEN 1
               WHEN I."FTC_TIPO_CLIENTE" = 'Afiliado' THEN 714
               WHEN I."FTC_TIPO_CLIENTE" = 'Asignado' THEN 713
            END
            INNER JOIN "MAESTROS"."TCGESPRO_INDICADOR_ESTADO_CUENTA" TIEC
            ON TIEC."FTN_ID_INDICADOR_ESTADO_CUENTA" = D."FCN_ID_INDICADOR_BONO"
            AND TIEC."FTN_VALOR" = CASE I."FTB_BONO"
               WHEN false THEN  1
               WHEN true THEN 2 END
        ...
        CASE WHEN R."FCN_ID_TIPO_SUBCTA" = ANY(C."FTA_SUBCUENTAS") AND C."FTC_SECCION" <> 'SDO' AND R."FCN_ID_PERIODO" = periodos.PERIODO_INICIAL THEN R."FTF_SALDO_INICIAL" ELSE 0 END::numeric(16, 2) AS "FTN_SALDO_ANTERIOR",
        ...
        FROM "MAESTROS"."TTGESPRO_CONFIGURACION_FORMATO_ESTADO_CUENTA" F
            INNER JOIN "MAESTROS"."TCGESPRO_CONFIGURACION_ANVERSO" C ON F."FCN_ID_GENERACION" = C."FCN_GENERACION"
            INNER JOIN "MAESTROS"."TCGESPRO_PERIODICIDAD" P ON F."FCN_ID_PERIODICIDAD_GENERACION" = P."FTN_ID_PERIODICIDAD"
            INNER JOIN "MAESTROS"."TCGESPRO_PERIODICIDAD" PG ON F."FCN_ID_PERIODICIDAD_ANVERSO" = PG."FTN_ID_PERIODICIDAD"
            ...
            INNER JOIN "MAESTROS"."TCGESPRO_PERIODO" T ON R."FCN_ID_PERIODO" = T."FTN_ID_PERIODO"
            INNER JOIN "MAESTROS"."TCGESPRO_PERIODO" PR ON PR."FTN_ID_PERIODO" = :term
        ...
        anverso_cuatrimestral_df = _create_spark_dataframe(spark, configure_postgres_oci_spark, f"""
        WITH data as (
        SELECT
        ...
        cast(C.FCN_NUMERO_CUENTA as bigint) as FCN_NUMERO_CUENTA,C.FTD_FECHA_CORTE,C.FTD_FECHA_GRAL_INICIO,C.FTD_FECHA_GRAL_FIN,C.FTD_FECHA_MOV_INICIO,
        ...
        print("reverso")
        _write_spark_dataframe(reverso_df, configure_postgres_oci_spark, '"ESTADO_CUENTA"."TTMUESTR_REVERSO"')
        print("anverso")
        _write_spark_dataframe(anverso_df, configure_postgres_oci_spark, '"ESTADO_CUENTA"."TTMUESTR_ANVERSO"')
        print("general")
        _write_spark_dataframe(general_df, configure_postgres_oci_spark, '"ESTADO_CUENTA"."TTMUESTR_GENERAL"')
