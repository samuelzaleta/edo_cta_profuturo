...
with define_extraction(phase, area, postgres_pool, postgres_pool) as (postgres, _):
    ...
    with register_time(postgres_pool, phase, term_id, user, area):
        ...
        formatos = general_df.select("FTN_ID_FORMATO").distinct().rdd.flatMap(lambda x: x).collect()
        tupleFormatos = ",".join(map(str, formatos))
        print(tupleFormatos)
        ...
        read_table_insert_temp_view(configure_postgres_spark, """
                    SELECT
                    REPLACE(PRD."FTC_PERIODO", '/','') AS "PERIODO",
                    F."FCN_ID_FORMATO_ESTADO_CUENTA" AS "FTN_ID_FORMATO",
                    ...
                    INNER JOIN "GESTOR"."TCGESPRO_PERIODO" PRD ON PRD."FTN_ID_PERIODO" = 1 --:term
                    WHERE
                    F."FCN_ID_FORMATO_ESTADO_CUENTA" IN (:formato)
                    AND mod(extract(MONTH FROM to_date(T."FTC_PERIODO", 'MM/YYYY')), P."FTN_MESES") = 0
                    AND to_date(T."FTC_PERIODO", 'MM/YYYY') BETWEEN :start - INTERVAL '1 month' * (PG."FTN_MESES" - 1) AND :end
                    GROUP BY  F."FCN_ID_FORMATO_ESTADO_CUENTA", PRD."FTC_PERIODO", F."FCN_ID_FORMATO_ESTADO_CUENTA", R."FCN_CUENTA", MC."FTN_ID_MOVIMIENTO_CONSAR", R."FTD_FEH_LIQUIDACION"
                    """, "edoCtaReverso", params={"term": term_id, "start": start_month, "end": end_month,"formato": tupleFormatos ,"user": str(user)})
        ...
        read_table_insert_temp_view(configure_postgres_spark, """
                ...
                AND to_date(T."FTC_PERIODO", 'MM/YYYY') BETWEEN :start - INTERVAL '1 month' * PG."FTN_MESES" AND :end
                GROUP BY 1
                ), SaldoFin AS (
                select T."FTN_ID_PERIODO",
                ...
                --"FTD_FECHA_FINAL"
                FROM dataset
        """, "edoCtaAnverso", params={"term": term_id, "start": start_month,"formato": tupleFormatos , "end": end_month, "user": str(user)})
