    ...
    with register_time(postgres_pool, phase, term_id, user, area):
        read_table_insert_temp_view(configure_postgres_spark, """
           ...
           DATE '2023-01-31' - INTERVAL '1 month' * (PA."FTN_MESES" - 1) AS "FTD_FECHA_MOV_INICIO",
            ...
            INNER JOIN "GESTOR"."TCGESPRO_FORMATO_ESTADO_CUENTA" FE
            ON F."FCN_ID_FORMATO_ESTADO_CUENTA" = FE."FTN_ID_FORMATO_ESTADO_CUENTA"
            ...
            INNER JOIN "HECHOS"."TCHECHOS_CLIENTE" I ON M."FCN_ID_PERIODO" = I."FCN_ID_PERIODO"
                                ...
                                AND F."FCN_ID_GENERACION" = CASE I."FTC_GENERACION" WHEN 'AFORE' THEN 2 WHEN 'TRANSICION' THEN 3 WHEN 'MIXTO' THEN 4 END
        ...
        read_table_insert_temp_view(configure_postgres_spark, """
        with dataset as (SELECT DISTINCT 
        F."FCN_ID_FORMATO_ESTADO_CUENTA" AS "FTN_ID_FORMATO",
        C."FTN_CUENTA"                   AS "FCN_NUMERO_CUENTA"
        FROM "GESTOR"."TTGESPRO_CONFIGURACION_FORMATO_ESTADO_CUENTA" F
        INNER JOIN "GESTOR"."TCGESPRO_FORMATO_ESTADO_CUENTA" FE
        ON F."FCN_ID_FORMATO_ESTADO_CUENTA" = FE."FTN_ID_FORMATO_ESTADO_CUENTA"
        INNER JOIN "GESTOR"."TCGESPRO_PERIODICIDAD" PG
        ON F."FCN_ID_PERIODICIDAD_GENERACION" = PG."FTN_ID_PERIODICIDAD"
        INNER JOIN "GESTOR"."TCGESPRO_PERIODICIDAD" PA
        ON F."FCN_ID_PERIODICIDAD_ANVERSO" = PA."FTN_ID_PERIODICIDAD"
        INNER JOIN "GESTOR"."TCGESPRO_PERIODICIDAD" PR
        ON F."FCN_ID_PERIODICIDAD_REVERSO" = PR."FTN_ID_PERIODICIDAD"
        INNER JOIN "GESTOR"."TCGESPRO_PERIODO" P ON P."FTN_ID_PERIODO" = 202301--:term
        INNER JOIN "GESTOR"."TCGESPRO_MUESTRA" M ON P."FTN_ID_PERIODO" = M."FCN_ID_PERIODO"
        INNER JOIN "HECHOS"."TCHECHOS_CLIENTE" I ON M."FCN_ID_PERIODO" = I."FCN_ID_PERIODO"
        AND M."FCN_CUENTA" = I."FCN_CUENTA"
        AND F."FCN_ID_GENERACION" =
        CASE I."FTC_GENERACION" WHEN 'AFORE' THEN 2 WHEN 'TRANSICION' THEN 3 WHEN 'MIXTO' THEN 4 END
        INNER JOIN "MAESTROS"."TCDATMAE_CLIENTE" C ON I."FCN_CUENTA" = C."FTN_CUENTA"
        ...
        --WHERE
        --mod(extract(MONTH FROM to_date(T."FTC_PERIODO", 'MM/YYYY')), P."FTN_MESES") = 0
        --AND
        --to_date(T."FTC_PERIODO", 'MM/YYYY') BETWEEN :start - INTERVAL '1 month' * (PG."FTN_MESES" - 1) AND :end
        GROUP BY
        ...
        read_table_insert_temp_view(configure_postgres_spark, """
            ...
            --AND
            to_date(T."FTC_PERIODO", 'MM/YYYY') BETWEEN DATE '2023-01-01'- INTERVAL '1 month' * PG."FTN_MESES" AND DATE '2023-01-31'
            GROUP BY 1
            ), SaldoFin AS (
            select T."FTN_ID_PERIODO",
            ...
            GROUP BY d."FCN_NUMERO_CUENTA", C."FTC_AHORRO", C."FTC_DES_CONCEPTO", C."FTC_SECCION",
